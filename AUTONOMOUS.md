...
- **bypassPermissions**: コンテナ内は承認なしで実行（**セキュリティの実効的な境界はコンテナ隔離・権限制御**が担う）
- **deny ルール**: 危険な操作をパターンで弾く**補助策**（事故防止）。突破可能であり防壁ではない（[SECURITY.md](SECURITY.md#境界boundaryと補助策guardの区別) 参照）
...
全ツール共通で、以下の操作を deny ルールで弾くよう設定しています:
...
> ⚠️ **これは「防壁」ではなく事故防止の「補助策」です。** deny は文字列パターンマッチであり、
> `/usr/bin/sudo`・シェル経由・別名・インタプリタ経由などで原理的に回避され得ます。また API キーは
> **環境変数**として渡るため `.env` 読み取り deny とは無関係に参照できます。**敵対的・侵害済みの
> エージェントを止める役割は、コンテナ隔離・権限境界（非 root + `no-new-privileges` + `cap_drop`）が
> 担います。** 詳細と実行時のデータ流出リスクは
> [SECURITY.md](SECURITY.md#境界boundaryと補助策guardの区別) を参照してください。
